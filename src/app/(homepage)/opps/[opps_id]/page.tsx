...
export default function Page ({ params }: { params: { opps_id: string } }) {
  ...
  const [opp, setOpp] = useState<OppType>({id:"", title:"", content:"", notified:false})
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center p-2 md:m-4 max-w-[960px]" id="top">
      ...
      <div className="p-4 md:m-6 w-full md:w-3/4 flex flex-col justify-between">
        <div>
        <h1 className="text-3xl font-bold text-center mb-8">{opp.title}</h1>
        <div dangerouslySetInnerHTML={{__html: opp.content}} />

        {opp.created_time &&
        <div className="text-right text-sm font-light mt-2 text-slate-400">发布于{convertTimestamp(opp.created_time)}</div>
        }
        </div>
        <div className="bottom-20 flex flex-row justify-around items-center mt-16">
          ...
          <button className="py-1 px-6"><a href="/opps">返回列表</a></button>
        </div>
      </div>
      :
      <Loading />
