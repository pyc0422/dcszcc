  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap flex-row flex-wrap justify-evenly">
        {opps?.length
          ? opps.map((op) => {
              const imgs = getImg(op.content);
              return (
                <Link
                  href={`/opps/${op.id}`}
                  key={op.id}
                  className="md:h-36 md:w-96 shadow-lg rounded-md py-2 m-2 flex flex-row justify-evenly items-center bg-blue-200/50 hover:scale-110 ease-in-out duration-300"
                >
